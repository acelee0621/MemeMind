...
class QueryService:
    ...
    async def _search_vector_db_async(
        ...
        try:
            ...
            logger.info(
                f"从 ChromaDB 检索到 {len(retrieved_pg_ids)} 个文本块 ID: {retrieved_pg_ids}"
            )
    ...
    async def retrieve_relevant_chunks(
        ...
        """
        logger.info(
            f"开始为查询 '{query_text[:100]}...' 检索相关的文本块 (top_k={top_k})"
        )
        ...
        # 2. 在向量数据库中搜索相关文本块的ID
        try:
            relevant_chunk_pg_ids = await self._search_vector_db_async(
        ...
        try:
            ...
            logger.info(
                f"已从 PostgreSQL 获取 {len(chunk_responses)} 个文本块的详细信息。"
            )
            ...
            if chunk_responses:
                ...
                if len(ordered_responses) != len(chunk_responses):
                    logger.warning(
                        "重新排序后，文本块数量与从数据库获取的数量不一致，可能存在ID丢失或重复。"
                    )
