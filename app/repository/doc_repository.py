...
class SourceDocumentRepository:
    ...
    async def create(self, data: SourceDocumentCreate) -> SourceDocument:
        ...
        try:
            ...
            logger.success(
                f"成功创建文档记录 ID: {new_document.id}, 路径: '{new_document.file_path}'"
            )
            return new_document
            ...
            raise AlreadyExistsException(f"具有路径 '{data.file_path}' 的文档已存在。")
    ...
    async def get_all(
        ...
        """获取所有文档记录（支持分页和排序）。"""
        logger.debug(
            f"正在查询文档列表，limit={limit}, offset={offset}, order_by='{order_by}'"
        )
    ...
    async def update(
        ...
        """更新一个已存在的文档记录。"""
        logger.info(
            f"正在更新文档 ID: {document_id}，数据: {data.model_dump(exclude_unset=True)}"
        )
        document = await self.get_by_id(
            document_id
        )  # 复用get_by_id以包含其日志和错误处理
